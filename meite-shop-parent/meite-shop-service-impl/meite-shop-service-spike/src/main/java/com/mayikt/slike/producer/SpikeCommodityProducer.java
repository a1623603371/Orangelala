package com.mayikt.slike.producer;
 ...
 * @ProjectName: 秒杀生产者
 * @Package: com.mayikt.slike.producer
 * @ClassName: SpilkeCommodityProducer
...
public class SpikeCommodityProducer implements RabbitTemplate.ConfirmCallback {
    ...
    public void send(JSONObject jsonObject){
        ...
        System.out.println("jsonObeject"+jsonObject);
        String messAgeId= UUID.randomUUID().toString().replace("-","");
        //封装消息
        Message message= MessageBuilder.withBody(jsonString.getBytes()).setContentType(MessageProperties.CONTENT_TYPE_JSON).setContentEncoding("utf-8").setMessageId(messAgeId)
                .build();
        //构建回调返回的数据（消息id）
