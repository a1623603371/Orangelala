package com.mayikt.pay.compensation.strategy.impl;

import com.mayikt.pay.compensation.PaymentCompensationStrategy;
...
/**
 * @ProjectName: 银联支付对账接口
 * @Package: com.mayikt.pay.compensation.strategy.impl
 * @ClassName: UnionPayCompensationStrategy
 * @Author: Administrator
 * @Description: ${description}
 * @Date: 2019/8/24 14:54
 * @Version: 1.0
 */
@Component
public class UnionPayCompensationStrategy implements PaymentCompensationStrategy {
    ...
    public Boolean payMentCompensation(PaymentTransactionEntity paymentTransactionEntity, PaymentChannelEntity paymentChannelEntity) {
        //1.商户号码
        String merchantId=paymentChannelEntity.getMerchantId();
        //2.下单时间
        Date createdTime=paymentTransactionEntity.getCreatedTime();
        //3.支付id
        String paymentId=paymentTransactionEntity.getPaymentId();
        ///4.调用银联支付接口
        Boolean uionPayCompensation=unionPayCompensation(paymentId,format(createdTime),merchantId);

        return uionPayCompensation;
    }
